import random
#Data for country and regions
# European countries: https://stevendobbelaerebe.wordpress.com/2009/03/03/array-with-a-list-of-european-countries/
...
regionDict = {'Africa': ["Djibouti", "Seychelles", "DR Congo", "Sierra Leone", "South Africa", "Egypt"],
             'Americas': ["United States", "Canada", "Mexico", "Bolivia","Colombia","Peru","Brazil", "Argentina", "Chile","Cuba","Suriname"],
             'Europe' : ["Albania","Andorra","Armenia","Austria","Azerbaijan","Belarus","Belgium","Bosnia and Herzegovina","Bulgaria","Croatia","Cyprus","Czech Republic","Denmark","Estonia","Finland","France","Georgia","Germany","Greece","Hungary","Iceland","Ireland","Italy","Kosovo","Latvia","Liechtenstein","Lithuania","Luxembourg","Macedonia","Malta","Moldova","Monaco","Montenegro","The Netherlands","Norway","Poland","Portugal","Romania","Russia","San Marino","Serbia","Slovakia","Slovenia","Spain","Sweden","Switzerland","Turkey","Ukraine","United Kingdom","Vatican City","Vietnam", "Myanmar", "Thailand", "Laos", "Cambodia", "Malaysia","Singapore","Philippines","Indonesia"],
             'Eastern Mediterranean': ["Afghanistan", "Iran ", "Jordan", "Kuwait", "UAE", "Lebanon"],
             'South-East Asia': ["Vietnam", "Myanmar", "Thailand", "Laos", "Cambodia", "Malaysia","Singapore","Philippines","Indonesia"],
             'Western Pacific': ["Australia","China","New Zealand", "Japan"]}

countries = regionDict['Africa'] + regionDict['Americas']+ regionDict[ 'Europe']+ regionDict['Eastern Mediterranean']+ regionDict['South-East Asia']+ regionDict[ 'Western Pacific']

countryPop = dict()
for country in countries:
    countryPop[country] = random.randint(1000000,900000000)
#Data for users
# fName array: https://gist.github.com/ruanbekker/a1506f06aa1df06c5a9501cb393626ea
# lName array: https://gist.github.com/benhorgen/4494868
...
def genDate(yearStart, yearEnd):
    ...
    if day < 10:
        day = '0'+str(day)
    return f"{random.randint(yearStart,yearEnd)}-{month}-{day}"
...
for region in regions:
    for country in regionDict[region]:
        if first:
            f.write(f"({regionID[region]},NULL,\'{country}\')")
            first = False
        else:
            f.write(f",\n({regionID[region]},NULL,\'{country}\')")
f.write(";\n\n")


#Generating proStaTer information
f.write("\n\nINSERT INTO proStaTer VALUES\n")
first = True
numOfProvinces = 10
for country in countries:
    for i in range(numOfProvinces):
        provincePop = (int)(countryPop[country]/numOfProvinces)
        if first:
            f.write(f"({countries.index(country) +1},NULL,\'province{i}\',{provincePop})")
            first = False
        else:
            f.write(f",\n({countries.index(country) +1},NULL,\'province{i}\',{provincePop})")
f.write(";\n\n")
...
for x in range(len(countries)):
    numInfec = random.randint(100000,1500000)
    countryNumInfections.append(random.randint(100000,1500000))
    countryNumDeaths.append(int(numInfec* random.uniform(0,0.5)))
    

f.write("\n\nINSERT INTO covid_19stat VALUES\n")
first = True
for pstID in range(len(countries)*numOfProvinces):
    i = pstID%len(countryNumInfections)
    date = genDate(2020,2022)
    provinceInfections = (int)(countryNumInfections[i]/numOfProvinces)
    provinceDeaths = (int)(countryNumDeaths[i]/numOfProvinces)
    if first:
        f.write(f"(NULL,{pstID+1},\'{date}\',{provinceInfections},{provinceDeaths})")
        first = False
    else:
        f.write(f",\n(NULL,{pstID+1},\'{date}\',{provinceInfections},{provinceDeaths})")
f.write(";\n\n")
...
first = True
for x in range(numOfProvinces*len(countries)):
    i = x%len(countries)
    statID = x+1
    vID = random.randint(1,4)
    numVaccinations = int(random.uniform(0.1,0.9)*countryPop[countries[i]])
...
privilege = ["Administrator","Researcher","Organization Delegate","Regular"]
researchInst = ["MIT","MCGILL","CONCORDIA","JOHN HOPKINS", "TORONTO UNIVERSITY"]
userDetails = dict()
...
for x in range(numUsers):
    ...
    password = random.randint(11111111,99999999)
    if first:
        userDetails[1] = [f"Joe Smith", countries.index("Canada"), 'joesmith@gmail.com',phone,date,username,password]
        f.write(f"(NULL,{cID},\'Joe\',\'Smith\',{phone},\'joesmith@gmail.com\',\'{date}\',\'JoeSmith\',\'{password}\')")
        first = False
    else:
        userDetails[x+1] = [f"{firstName} {lastName}", cID, email,phone,date,username,password]
        f.write(f",\n(NULL,{cID},\'{firstName}\',\'{lastName}\',{phone},\'{email}\',\'{date}\',\'{username}\',\'{password}\')")
            
f.write(";\n\n")

f.write("\n\nINSERT INTO isSuspended VALUES\n")
first = True
numSuspended = int(0.2*numUsers)
suspended = [-1]
uID = -1
for x in range(numSuspended):
    while uID in suspended:
...
first = True
allOrgs = orgIDS + rIDS
for oID,iD in enumerate(allOrgs):
    if iD != -1:
        if iD in orgIDS:
            ...
            if first:
                f.write(f"({oID},\'{iD}\',NULL,\'{name}\',\'{oType}\')")
                first = False
            else:
                f.write(f",\n({oID},\'{iD}\',NULL,\'{name}\',\'{oType}\')")
        else:
            name = f"National Research Institute"
            oType = "Research Center"
            if first:
                f.write(f"({oID},NULL,\'{iD}\',\'{name}\',\'{oType}\')")
                first = False
            else:
                f.write(f",\n({oID},NULL,\'{iD}\',\'{name}\',\'{oType}\')")
f.write(";\n\n")
...
count = 0
for i in range(1,numUsers):
    if (i in rIDS) and (random.uniform(0,1) > 0.5):
        count +=1
        authors[count] = i
        if first:
            f.write(f"(NULL,{i},NULL)")
            first = False
        else:
            f.write(f",\n(NULL,{i},NULL)") 
    
    elif (i in orgIDS) and (random.uniform(0,1) > 0.5):
        count +=1
        authors[count] = i
        if first:
            f.write(f"(NULL,NULL,{i})")
            first = False
        else:
            f.write(f",\n(NULL,NULL,{i})") 
f.write(";\n\n")
...
allOrgs.remove(-1)
allOrgs.remove(-1)
authIDS = list(authors.keys())
for x in range(numArticles):
    orgDel = False
    rDel = False
    authorID = authIDS[random.randint(1,len(authIDS)-1)]
    if authors[authorID] in orgIDS:    
        author =  f"{countries[userDetails[authors[authorID]][1]]} Government Agency"
        orgDel = True
    else:
        author = userDetails[authors[authorID]][0]
        rDel = True
    majorTopic = 'MajorTop'
    minorTopic = "MinorTop"
    ...
    if first:
        if orgDel:  
            f.write(f"(NULL,{authorID},\'Joe Smith\',\'{majorTopic}\',\'{minorTopic}\',\'{summary}\',\'{content}\',\'{date}\')")
        else:
            f.write(f"(NULL,{authorID},\'Joe Smith\',\'{majorTopic}\',\'{minorTopic}\',\'{summary}\',\'{content}\',\'{date}\')")
        first = False
    else:
        if orgDel:
            f.write(f",\n(NULL,{authorID},\'{author}\',\'{majorTopic}\',\'{minorTopic}\',\'{summary}\',\'{content}\',\'{date}\')")
        else:
            f.write(f",\n(NULL,{authorID},\'{author}\',\'{majorTopic}\',\'{minorTopic}\',\'{summary}\',\'{content}\',\'{date}\')")            
f.write(";\n\n")
...
first = True
numArticles = 100
for aID in range(1,numArticles):
    date = genDate(2020,2022)
...
for uID in range(numUsers):
    if random.uniform(0,1) > 0.8:
        authorID = authIDS[random.randint(1,len(authIDS)-1)]
        subscriptions[uID] = authorID
        if first:
            f.write(f"({uID}, {authorID})")
            first = False
        else:
            f.write(f",\n({uID}, {authorID})")  
f.write(";\n\n")
...
for uID in range(numUsers):
    if uID in subscriptions:
        authorID = subscriptions[uID]
        aID = random.randint(1,numArticles)
        date_time = genDate(2020,2022)
        if first:
            f.write(f"({uID}, {authorID}, NULL, {aID},\'{date_time}\',\'Subject of the email\' )")
            first = False
        else:
            f.write(f",\n({uID}, {authorID}, NULL, {aID},\'{date_time}\',\'Subject of the email\' )")  
f.write(";\n\n")
