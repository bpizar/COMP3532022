...
first = True
for statID in range(10*len(countries)):
    i = statID%len(countries)
    date = genDate(2020,2022)
    if first:
        f.write(f"(NULL,{i+1},\'{date}\',{countryNumInfections[i]},{countryNumDeaths[i]})")
        first = False
    else:
...
for x in range(numArticles):
    ...
    if rand > 0.5:
        uID = orgIDS[random.randint(1,len(orgIDS)-1)]
        author =  f"{countries[userDetails[uID][1]]} Government Agency"
        orgDel = True
    else:
        uID = rIDS[random.randint(1,len(rIDS)-1)]
    ...
    date = genDate(2020,2022)
    if first:
        f.write(f"(NULL,1,NULL,\'Joe Smith\',\'{majorTopic}\',\'{minorTopic}\',\'{summary}\',\'{content}\',\'{date}\')")
        first = False
    else:
        if orgDel:
